		...
		return {
            computeStats : function(item) {
                ...
                if (["linear","nominal","continuous"].indexOf(item.domain) > -1) {
                    ...
                    var queryResult = domainsStore.query({name : item.domain})[0];
            ...
            nominal : function(name, parameters) { 
                ...
                return [{id : "Item counts", value : ""}].concat(parameters.map(function(parameter) {
                    var query = {}; 
            ...
            continuous : function(name, parameters) { 
                var dataset = eventsStore.query({}).map(function(x) {return parseFloat(x[name]);});
                var min = parseFloat(parameters[0]), max = parseFloat(parameters[1]);
                return [{id : "Values range", value : ""}].concat([{id:"min", value:min},{id:"max", value:max}]);
            }
        };
});
